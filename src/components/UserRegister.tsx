...
export default function UserRegister() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!displayName.trim() || !email.trim() || !phone.trim() || !password || !confirmPassword) {
    ...
    try {
      ...
      const res = await fetch("/api/user-register", {
        ...
        body: JSON.stringify({
          ...
          phone,
          country: countryName,
          country_code: selectedCallingCode,
        }),
      });
    ...
    <div className="min-h-screen bg-[#020617] text-white flex items-center justify-center p-6 selection:bg-amber-500/30">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="border border-white/10 bg-white/5 rounded-[2.5rem] p-8 shadow-2xl space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-black uppercase tracking-tight">Create Account</h1>
            </div>
            <button
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">or register with email</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs font-bold text-white/60">
                {selectedCallingCode}
              </div>
            </div>
        ...
        <div className="text-center">
          ...
          >
            Already have an account? <span className="text-amber-500">Login</span>
          </button>
        </div>
      </div>
    </div>
